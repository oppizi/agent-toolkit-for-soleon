...
- Live deployment happens only at Step 7, behind explicit user confirmation, via the `soleon` MCP server (stateless HTTP, bearer-JWT auth). Everything before it is offline. The deterministic converter still stops at validated JSON — the deploy call is the LLM head's job, not the converter's.
